...
class BDD100K5Dataset(Dataset):
    ...
    def __init__(self, base_dir='../data/bdd100k', split='train',
                 ...
                 net_type='unet', ignore_index=0, debug=False):
        self.debug = debug
        ...
        self.img_paths = sorted(self.base_dir.glob(f'images/seg_lane/{self.split}/*.jpg'))
        self.lbl_paths = sorted(self.base_dir.glob(f'drivable_lane_seg_maps/labels/{self.split}/*.png'))
...
if __name__ == '__main__':
    ...
    dataset = BDD100K5Dataset(split='train', net_type='deeplab', ignore_index=0, debug=True,
                              affine_augmenter=affine_augmenter, image_augmenter=image_augmenter,
                              target_size=(1280, 1280))
    dataloader = DataLoader(dataset, batch_size=8, shuffle=True)
    palette = np.array([    
	[0, 0, 0], # 0 black background
        [0, 0, 255], # 1 red directly drivable area
        [255, 0, 0], # 2 blue alternatively drivable area
        [255, 0, 255], # 3 pink lane
        [255, 100, 100], # 4 person
        [0, 0, 100], # 5  car
        [50, 0, 100], # 6 bus
        [0, 102, 102], # 7 caravan
        [0, 102, 51], # 8 motorcycle
        [102, 0, 51]]) # 9 trucK
    ...
    for i, batched in enumerate(dataloader):
        ...
        if i == 0:
            ...
            for j in range(8):
                ...
                axes[j][1].set_xticks([])
                axes[j][1].set_yticks([])
            plt.savefig('dataset/bdd100k5.png')
            plt.close()
        break
