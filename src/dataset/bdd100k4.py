...
class BDD100K4Dataset(Dataset):
    n_classes = 12
    void_classes = [1, 2, 3, 4, 5, 8, 9, 10, 255]
    valid_classes = [0, 6, 7, 11, 12, 13, 14, 15, 16, 17, 18, 19]
    # 0 : road
    # 6 : traffic light
    # 7 : traffic sign
    # 11 : person
    ...
    def __init__(self, base_dir='../data/bdd100k/seg', split='train',
                 affine_augmenter=None, image_augmenter=None, target_size=(1024, 1024),
                 net_type='deeplab', ignore_index=19, debug=False):
        self.debug = debug
        ...
        self.lbl_paths = sorted(self.base_dir.glob(f'labels/{self.split}/*.png'))
...
if __name__ == '__main__':
    ...
    dataset = BDD100K4Dataset(split='train', net_type='deeplab', ignore_index=19, debug=True,
                              affine_augmenter=affine_augmenter, image_augmenter=image_augmenter,
                              target_size=(1280, 1280))
    dataloader = DataLoader(dataset, batch_size=8, shuffle=True)
    palette = np.array([[128, 64, 128],  # road
    ...
    for i, batched in enumerate(dataloader):
        ...
        if i == 0:
            ...
            for j in range(8):
                ...
                axes[j][1].set_xticks([])
                axes[j][1].set_yticks([])
            plt.savefig('dataset/bdd100k4.png')
            plt.close()
        break
